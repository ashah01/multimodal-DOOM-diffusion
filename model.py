...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        self, n_channels: int, n_heads: int = 1, d_k: int = None, n_groups: int = 16
    ):
        """
...
class TextTimeEmbedding(nn.Module):
    def __init__(self, encoder_dim: int, time_embed_dim: int, num_heads: int = 8):
        super().__init__()
...
class UNet(nn.Module):
    ...
    def __init__(
        self,
        # every image now has the previous image stacked
        image_channels: int = 3 * 2,
        n_channels: int = 32,
        ...
        * `n_blocks` is the number of `UpDownBlocks` at each resolution
        ...
        self.encoder_hid_proj = nn.Linear(4, n_channels * 4)  # 4 possible actions
        self.text_emb = TextTimeEmbedding(128, 128, 8)
        ...
        self.final = nn.Conv2d(
            in_channels, 3, kernel_size=(3, 3), padding=(1, 1)
        )
